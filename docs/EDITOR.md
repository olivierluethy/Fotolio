...
- Live DOM patches for cheap changes (overlay text, position/align/height/parallax/backdrop, nav in/out & reorder); a fast server **fragment** re-render (`hero`/`subnav`/`main`) for structural changes (image sets, slideshow mode, blocks). Hero **image** changes do a full canvas reload so the top nav's over-hero treatment tracks a hero appearing/disappearing.
- Editing a gallery's or page's hero **materialises** a `header_config` override from its current auto-hero on first change; from then on that hero is explicit (no longer auto-derived from the first photo / gallery name).
- The "overlay style" control is implemented as a **text backdrop** toggle (a translucent panel behind the hero text) — a visible, useful effect rather than a no-op, since the theme renders hero text white over a gradient.
- Edit token TTL is 8h (spans a session); preview token 24h. Long-idle sessions past 8h need a page refresh to mint a new edit token.
- **Undo/redo** captures a whole-draft snapshot at the single `mutate()` choke point. Rapid text edits coalesce (600ms window) into one step; structural ops each step individually. Toolbar buttons + Cmd/Ctrl+Z / Shift+Cmd/Ctrl+Z; the canvas iframe forwards the shortcut (except while a text field has the caret) via a `shortcut` bridge message. Undo/redo do a full canvas reload so structural changes reflect reliably.
- **Global design + Custom CSS** live in `site.settings.accent` / `site.settings.custom_css` (in the draft, so they publish with everything else). Layout injects both; `settings.accent` wins over the legacy `sites.accent` column. Custom CSS is scoped to the site document (never the dashboard shell) and has `</style>` break-out neutralised. Both preview live via bridge messages (`accent`, `custom-css`) — no full reload while dragging a colour or typing CSS.
- **Rotating words** are now directly selectable: each phrase is `contenteditable` in the canvas (`data-editable="rotating:{i}"`) and edits fold into `hero.rotating_words` live. Edit mode lays every phrase out static/visible/clickable (`.rotator[data-rotator-edit]`) since the theme stacks them absolutely and animates them. The Hero panel has an add/remove phrase-list editor.
- **Analytics** is a separate subsystem — see `docs/ANALYTICS.md`. The beacon is injected only in `live` mode (never edit/preview) so editing never pollutes visitor stats.
- Mobile: the visual canvas is desktop-first; below `lg` the editor shows a "open on a larger screen" note. Photo library management and per-photo metadata stay in the existing **Library** page.
- **Deploy:** no SSH host is configured in this environment, so live PROD was skipped (consistent with the project's standing infra decision). `scripts/deploy.sh` already runs `php bin/console migrate` (picks up `008` + backfill) and rebuilds the SPA, so a host deploy needs no script change.
